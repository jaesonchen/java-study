...
**当add 第1个元素时，minCapacity为1，在Math.max()方法比较后，minCapacity 为10。**
...
- 当 add 第1个元素到 ArrayList 时，elementData.length 为0 （因为还是一个空的 list），因为执行了 `ensureCapacityInternal()` 方法 ，所以 minCapacity 此时为10。此时，`minCapacity - elementData.length > 0 `成立，所以会进入 `grow(minCapacity)` 方法。
- 当add 第2个元素时，minCapacity 为2，此时elementData.length(容量)在添加第一个元素后扩容成 10 了。此时，`minCapacity - elementData.length > 0 ` 不成立，所以不会进入 （执行）`grow(minCapacity)` 方法。
...
### `System.arraycopy()` 方法
`System.arraycopy()` 复制源数组到目标数组，指定起始索引和复制长度，如果源和目标是同一个数组，则复制前会先使用一个临时数组保存要复制的子序列。
...
### `Arrays.copyOf()`方法
`Arrays.copyOf()` 会新建一个数组，并复制指定长度的数据到新数组。
