...
/**
 * 
 * java虚拟机实现必须在每个类或接口被java程序“首次主动使用”时才初始化他们；被动使用不会初始化类，但是有可能会加载类。
 * jvm在加载的过程中，如果遇到了.class文件的缺失或者存在错误，类加载器只会在首次主动使用它们时才会报错，如果一直没有主动使用，则不会报错。
 *               
 * - 主动使用：
 *  1. 创建类的实例（new /Class.newInstance()）
 *  2. 访问某个类或者接口的非final类型的静态变量/对该静态变量赋值、调用类的静态方法
 *  3. 初始化一个类的子类
 *  4. jvm启动时被标明为启动类的类（main方法所在的类）
 *               
 *               
 * @author       zq
