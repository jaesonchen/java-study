...
假设userid索引按排序顺序有 [10, 20, 30, 40]几个字段，Innodb将索引数据分成几个区间(-∞, 10], (10, 20], (20, 30], (30, 40], (40, ∞)几个区间，   
`update set name='chenzq' where userid=30`    
不仅用行锁锁住了相应的数据行索引userid=30；同时也在索引两边的区间，(20, 30]和 (30, 40]都加入了gap锁。这样其他事务就无法在这个两个区间insert进新数据。   
